...
import { coordsToCommunity, findAreaName, searchLocation } from "../utils/maps.js";
 ...
 * @param {import("express").Response} res
 */
export async function getCommunityByLngLat(req, res) {
  const { data, success, error } = LngLat.safeParse(req.query);
  ...
  const area = await coordsToCommunity([lng, lat])
